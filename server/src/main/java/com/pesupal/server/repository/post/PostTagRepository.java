...
public interface PostTagRepository extends JpaRepository<PostTag, Long> {
    ...
    @Query("""
            SELECT pt.tag.name AS name, COUNT(pt.post.id) AS count
                  FROM PostTag pt
            WHERE pt.post.org = :org
            GROUP BY pt.tag
            ORDER BY COUNT(pt.post.id) DESC, pt.tag.name ASC
            """)
    Page<PostTagProjection> findAllByPost_OrgOrderByCount(Org org, Pageable pageable);
}
