import './BookmarkPostsLayout.css'
...
const NoPostsAvailable = () => {
    ...
    return (
        <div className='FCCC w100 h100' id='no-data-found'>
            ...
            <p className='w100 alignCenter'>Start bookmarking posts to see them here.</p>
        </div>
    )
}

const BookmarkPostsLayout = () => {
    ...
    useEffect(() => {
        ...
        apiRequest(`/api/v1/post/bookmarks?page=${page}&size=${size}&sort_order=${sortOrder}`, 'GET').then(({ data, info }) => {
            setLoading(false);
    ...
    const overlayClickHandler = (e) => {
        if (e.target.id === 'bookmarked-posts-layout') {
            dispatch(setActivePostId(null)); // Close the active post options when clicking outside
        }
    }

    return (
        <div id='bookmarked-posts-layout' className='posts-layout FCCS w100 h100' onClick={overlayClickHandler}>
            <div id="postsList">
                {loading ? <Loader /> :
                    error ? <ErrorMessage /> :
...
export default BookmarkPostsLayout