...
const CustomModulesItem = ({ item }) => {
    const { name, icon, icon_color, route, created_info, update_permission } = item,
        { created_by, created_date } = created_info;
    return (
        <Link to={route} className='CustomModulesItem mR10 FRCC' title={"Created by: " + created_by + " on " + created_date}>
            <i className={icon + " pR10 img_20_20"} style={{ color: icon_color }}></i>
